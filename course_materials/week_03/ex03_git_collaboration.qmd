...
::: {style="width:56px; height:56px; border-radius:50%; border:2px solid #e74c3c; flex-shrink:0; background-image:url('../../assets/img/banner_course.png'); background-size:50%; background-position:25% 35%;"}
:::
...
::: {.callout-note}
*Note*: this part is more advanced and not required for the course. It is however an important workflow to understand and master for any collaborative project.
:::
...
# If there are conflicts, Git will tell you. Fix them, the run:
git add .
git commit
```
...
# repeat (Fix -> git add -> git rebase --continue) until until Git says something like "Successfully rebased and updated <feature-branch>."
```
